...
export default function RecipeCard({ recipe }: { recipe: RecipeCardData }) {
  ...
  return (
    <Link
      ...
      className="group block overflow-hidden rounded-2xl border border-ink-100 bg-white shadow-sm transition hover:shadow-md"
    >
      <div className="relative aspect-[4/3] w-full overflow-hidden bg-ink-100">
        ...
        />
        {recipe.isStreetFood && (
          <span className="absolute left-2 top-2 rounded-full bg-white/90 px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide text-chili-600 shadow-sm">
            Street Food
          </span>
        )}
      </div>
      <div className="p-4">
        ...
        <div className="mt-2 flex items-center justify-between text-xs text-ink-400">
          ...
          <span className="rounded-full bg-leaf-50 px-2 py-0.5 font-medium text-leaf-700">
